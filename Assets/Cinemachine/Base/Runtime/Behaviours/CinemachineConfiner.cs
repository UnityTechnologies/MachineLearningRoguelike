...
namespace Cinemachine
{
    ...
    public class CinemachineConfiner : CinemachineExtension
    {
        ...
        void ValidatePathCache()
        {
            if (m_BoundingShape2D == null)
                InvalidatePathCache();
            else if (m_pathCache == null || m_pathCache.Count != m_BoundingShape2D.pathCount)
            {
                m_pathCache = new List<List<Vector2>>();
                for (int i = 0; i < m_BoundingShape2D.pathCount; ++i)
                {
                    Vector2[] path = m_BoundingShape2D.GetPath(i);
